...
par(mar = c(0, 0, 1, 0))
sa %>% st_union() %>% st_geometry() %>% 
  plot(main = "Blue Bike Stations in Boston City",
       col = "cornsilk")
bstation %>% st_geometry() %>% 
  plot(pch = 20, col = "steelblue1", add = TRUE)
legend(x = "bottomright",
       legend = "Blue Bike Stations",
       pch = 20, col = "steelblue1", bty = "n")

# census <- dir(dataPath, pattern = ".csv", full.names = TRUE) %>% 
#   map(function(x) {
#     read_csv(x)
#   })
# names <- dir(dataPath, pattern = ".csv", full.names = TRUE) %>% basename() %>% 
#   strsplit(., "[.]") %>% unlist %>% delete("csv")
# names[1](1)
# l <- strsplit(names[1], "[.]")
# l[1,1]
# unlist(l)[1]
